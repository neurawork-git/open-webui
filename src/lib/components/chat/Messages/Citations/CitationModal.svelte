<script lang="ts">
	...
	function highlightKeywords(text: string, keywords: string[]): string {
		...
		for (const keyword of sortedKeywords) {
			...
			escaped = escaped.replace(regex, '$1<mark class="bg-yellow-200 dark:bg-yellow-700 text-yellow-900 dark:text-yellow-100 px-0.5 rounded">$2</mark>$3');
		}
			...
			>
				...
				{#if allKeywords.length > 0}
					<div class="flex flex-wrap items-center gap-2 mb-3 pb-2 border-b border-gray-200 dark:border-gray-700">
						<span class="text-xs text-gray-500 dark:text-gray-400">{$i18n.t('Matched keywords')}:</span>
						{#each allKeywords as keyword}
							<span class="text-xs px-1.5 py-0.5 bg-yellow-200 dark:bg-yellow-700 text-yellow-900 dark:text-yellow-100 rounded">
								{keyword}
							</span>
						{/each}
					</div>
					...
					<div class="flex flex-col w-full gap-2">
						...
						<div>
							...
							{:else}
								...
								{:else}
									<div class="text-sm dark:text-gray-400 whitespace-pre-line">{document.document
										.trim()
										.replace(/\n\n+/g, '\n\n')}</div>
								{/if}
							{/if}
