<script lang="ts">
	...
	const uploadFileHandler = async (file, process = true, itemData = {}) => {
		...
		const fileItem = {
			...
			// FORK: the fork's former unconditional full-context override is gone —
			// v0.11.0 ships this as a user setting; the fork only flips the unset
			// default from 'focused' to 'full' (see Settings/Interface.svelte).
			...(($settings?.defaultUploadContext ?? 'full') === 'full' ? { context: 'full' } : {}),
			...itemData
		};
