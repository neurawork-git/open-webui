<script lang="ts">
	...
	// File
	// FORK: fork instances default chat uploads to full-context injection
	// (upstream default is 'focused'). Users can still flip it below.
	let defaultUploadContext: 'full' | 'focused' = 'full';
	...
	onMount(async () => {
		...
		defaultUploadContext = $settings?.defaultUploadContext ?? 'full'; // FORK: see above
	});
</script>
