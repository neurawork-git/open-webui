<script lang="ts">
	...
	const submitHandler = async () => {
		...
		// FORK: save per-model RAG settings
	...
	onMount(async () => {
		...
		if (model) {
			...
			ragSettings = model?.meta?.rag_settings ?? {}; // FORK: per-model RAG settings
			...
			{#if !edit || (edit && model)}
				...
				>
					<div class="w-full px-1">
						...
						<!-- FORK: per-model RAG settings override -->
						<div class="my-3">
