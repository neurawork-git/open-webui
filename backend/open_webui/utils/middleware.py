...
async def chat_completion_files_handler(
    ...
    if files := body.get('metadata', {}).get('files', None):
        ...
        try:
            # `rag_config` is the single batched SELECT above (fork superset —
            # it also carries 'rag.enable_reranking'); upstream's second
            # get_many() here would have been a redundant round trip.
        ...
        if folder and folder.data:
            ...
            if 'files' in folder.data:
                ...
                if metadata.get('params', {}).get('function_calling') != 'legacy':
                    ...
                    # v0.11.0 security fix: entries are filtered against what the
                    # folder OWNER can still read (upstream replaced the former
                    # caller-side get_accessible_folder_files filter with this).
