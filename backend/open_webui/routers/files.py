...
async def process_uploaded_file(
    ...
    async def _process_handler(db_session):
        # FORK: one dashboard task per upload (best-effort). The same tracker is
        # passed into every _process_file call below so the extract step and the
        # knowledge-link step share a single row (one upload = one task).
        tracker = await _start_processing_tracker(file_item, user, file_metadata.get('knowledge_id'))
        try:
            ...
            # FORK: extraction (and any KB-link) succeeded → close the dashboard task.
            if tracker:
            ...
            if tracker:
                await _safe_track(tracker.fail(str(e.detail) if hasattr(e, 'detail') else str(e)))
            await Files.update_file_data_by_id(
                file_item.id,
                {
                    'status': 'failed',
