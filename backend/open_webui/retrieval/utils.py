...
async def query_collection(
    ...
    # (e.g. when get_last_user_message returns None) -- from upstream v0.9.2.
...
async def get_sources_from_items(
    ...
    # upstream call signature stays backward-compatible).
        ...
        if item.get("type") == "text":
            ...
            if knowledge_base and (
                ...
                if (
                    ...
                    # FORK: vector-search mode — query this KB independently with
                    # its own per-KB settings. Upstream's cross-tenant legacy
                    # collection-name validation guard is PRESERVED here (it must
                    # not regress to the fork's older unguarded body).
                    if item.get('legacy'):
                        if BYPASS_RETRIEVAL_ACCESS_CONTROL:
