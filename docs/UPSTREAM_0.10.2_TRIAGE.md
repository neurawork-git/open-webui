...
## 6. Nachtrag (2026-07-14, nach Umsetzung) — Korrekturen an dieser Triage

Die Migration wurde am 2026-07-14 auf `feature/owui-0.10.2` umgesetzt. Dabei stellten sich
zwei Triage-Annahmen als falsch bzw. unvollständig heraus:

1. **§4.2 (BLOB_PATH_TO_KEY-Datenverlust) — ENTWARNUNG.** `_walk_blob` in
   `3ff2c63645b8_reshape_config_to_per_key_rows.py` kopiert unbekannte Leaf-Pfade
   mit (`else: result[path] = value`), und der alte Blob bleibt als `config_old`
   erhalten. Da die alten ConfigVar-Blob-Pfade identisch mit den neuen dotted Keys
   sind, migrieren alle Fork-Configs automatisch. Keine Fork-Nachmigration nötig.
2. **Weitere Replay-Verluste entdeckt** (zusätzlich zum Chroma-Fix aus §4.1):
   - `ProcessingTaskTracker`-Hooks in `routers/retrieval.py` fehlen im Fork-HEAD;
     sie existieren nur als uncommitted WIP im Haupt-Working-Tree → nach dem Merge
     auf `feature/owui-0.10.2` rebasen.
   - Die per-KB-RAG-Settings-Verdrahtung in `utils/middleware.py`
     (`per_knowledge_rag_settings`-Kaskade) fehlte im Fork-HEAD; sie wurde aus dem
     alten `main` re-portiert und ist auf `feature/owui-0.10.2` wiederhergestellt.
3. **§4.1-Zusatz:** Kein Kunde nutzt Chroma (überall Supabase/Postgres) — der
   Chroma-Hotfix-Schritt entfiel; der Upstream-Fix kam über den Merge mit.
4. **External KBs:** nur direkte DB-Provider (qdrant/milvus/pgvector), kein
   REST-/API-Provider; Query-Embedding macht OWUI selbst (Modell muss zum externen
   Index passen); Quell-Links nur via konfigurierbarem `url_field` im Chunk-Payload.

Umsetzung: siehe `docs/FORK_CHANGES.md` (regeneriert gegen v0.10.2) und die
Commit-Serie auf `feature/owui-0.10.2`.
